...
public class WebDriverImpl implements HelperBase, WebDriverBase {
	...
	public void setHomeWindow(){
		this.homeWindow = driver.get().getWindowHandle();
	}
	...
	public void click(WebElement elt){
		waitUntilElementClickable(elt).click();
	...
	public List<WebElement> waitUntilAllElementsPrestent(By ele){
		return wait.get().until(ExpectedConditions.presenceOfAllElementsLocatedBy(ele));
	}
	...
	public void switchWindows(){
		...
		for (String curWindow : windowHandles){
			driver.get().switchTo().window(curWindow);
		}
	}

	public void switchToWindow(String windowToSwitch){
		driver.get().switchTo().window(windowToSwitch);
	}
}
